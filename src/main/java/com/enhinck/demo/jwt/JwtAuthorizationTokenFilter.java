...
public class JwtAuthorizationTokenFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		// 调试使用的用户id
		String debugUserName = request.getParameter("debugUserName");
		if (debugUserName != null) {
			UserDetails userDetails = this.userDetailsService.loadUserByUsername(debugUserName);
			if (userDetails != null) {
				UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
						userDetails, null, userDetails.getAuthorities());
				authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
				logger.info("debug user '{}', setting security context", debugUserName);
